...
import { TreePine, Waves, Lightbulb, Zap, Sparkles, CheckCircle } from "lucide-react";
...
export default function EpicSplash({ onComplete }: EpicSplashProps) {
  ...
  const phases = [
    { text: "تهيئة نظام اللاندسكيب الذكي", textEn: "Initializing Smart Landscape OS", duration: 800 },
    { text: "تحميل ستوديو الذكاء الاصطناعي", textEn: "Loading AI Design Studio", duration: 1000 },
    { text: "تفعيل الرادار التسويقي", textEn: "Activating Smart Marketing Radar", duration: 900 },
    { text: "ربط أنظمة الري الذكية", textEn: "Connecting Smart Irrigation Systems", duration: 700 },
    { text: "تحضير واجهة المستخدم", textEn: "Preparing User Interface", duration: 600 },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-emerald-900 via-teal-900 to-green-900 overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-20 right-20 w-80 h-80 bg-gradient-to-br from-green-400/15 to-emerald-400/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />

        {/* Dynamic Grid */}
        <div className={"absolute inset-0 bg-[url('data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 100 100\"><defs><pattern id=\"grid\" width=\"10\" height=\"10\" patternUnits=\"userSpaceOnUse\"><path d=\"M 10 0 L 0 0 0 10\" fill=\"none\" stroke=\"%23ffffff\" stroke-width=\"0.5\" opacity=\"0.1\"/></pattern></defs><rect width=\"100\" height=\"100\" fill=\"url(%23grid)\"/></svg>')] opacity-20"} />
      </div>
      ...
      <div className="relative z-10 text-center max-w-2xl mx-auto px-8">
        ...
        {/* Tagline */}
        <div className={`transition-all duration-1000 delay-500 ${showTagline ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        {/* Progress Section */}
        <div className={`transition-all duration-1000 delay-1000 ${showProgress ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {/* Current Phase */}
          <div className="mb-6">
          ...
          <div className="space-y-2">
            {phases.map((phase, index) => (
              <div key={index} className={`flex items-center gap-3 text-sm transition-all duration-300 ${
                index === currentPhase ? 'text-emerald-300 scale-105' :
                completedPhases.includes(index) ? 'text-green-400' : 'text-gray-400'
              }`}>
